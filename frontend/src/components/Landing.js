...
const Landing = () => {
    ...
    const [selectedClassItem, setSelectedClassItem] = useState(null); // Added state for selectedClassItem

    // Adjust handleOpen to set the selectedClassItem
    const handleOpen = (classItem) => {
        setSelectedClassItem(classItem); // Now this function call will work as expected
        setOpen(true);
    };

    const handleClose = () => {
        setOpen(false);
        setSelectedClassItem(null); // Clear the selected class item on close
    };
    ...
    // This function checks if a given date 'd' is a class day based on the class's start date, frequency, and days of the week the class occurs.
    function isThisAClassDay(d, classItem) {
        const { date, frequency, days } = classItem;
        const start = new Date(date); // Convert start date string to Date object
        const current = new Date(d); // Ensure 'd' is a Date object, not mutated

        // Check if the current date is before the start date
        if (current < start) {
            return false; // The class hasn't started yet
        }
        ...
        switch (frequency) {
            case 'none':
                return false; // No recurring classes
            case 'daily':
                return true; // Class occurs every day
            case 'weekly':
                // Check if the current day is one of the class days
                if (days.includes(current.getDay())) {
                    return true;
                }
                return false;
            case 'bi-weekly':
                // For bi-weekly, check if it's the correct week and one of the class days
                if (diffDays % 14 < 7 && days.includes(current.getDay())) {
                    return true;
                }
                return false;
            case 'monthly':
                // Class occurs once a month on the same day number
